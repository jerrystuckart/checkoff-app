...
/**
 * A minimal chainable Supabase query-builder stub: every method returns
 * itself, and it resolves like a promise. `.range(from, to)` actually
 * slices `result.data` (when it's an array) so tests can exercise
 * lib/supabasePagination.js's fetchAllRows() with a genuinely
 * multi-page fixture — see the >1000-row pagination regression test
 * below, which is the whole reason `.range()` needs to behave for real
 * here rather than being a no-op like the other chained methods.
 */
function chainable(result) {
  const handler = {
    ...
    order: () => handler,
    range: (from, to) => {
      const page = Array.isArray(result.data) ? result.data.slice(from, to + 1) : result.data
      return { then: (resolve, reject) => Promise.resolve({ ...result, data: page }).then(resolve, reject) }
    },
...
// ---------------------------------------------------------------------------
// eligibleLocalCount (2026-09-23 field bug fix — Munich screenshot
// regression): computed BEFORE the Home Rail exclusion loop, from the full
// within-radius eligibility pool — must never collapse to 0 just because
// personalization/dedup already excluded those same items from `candidates`.
// ---------------------------------------------------------------------------

test('REGRESSION (screenshot state): eligibleLocalCount reflects full local inventory even when EVERY eligible item is already on the Home Rail (candidates goes to 0)', async () => {
  const items = [
    makeItem('rail-1', { lat: 48.1374, lng: 11.5755 }),
    makeItem('rail-2', { lat: 48.1375, lng: 11.5756 }),
    makeItem('rail-3', { lat: 48.1376, lng: 11.5757 }),
  ]
  const MUNICH = { latitude: 48.1351, longitude: 11.5820 }
  const { client } = makeStubClient({ items, momentumRows: [] })
  const result = await assembleWhatsGoodCandidates({
    userId: USER_ID,
    userLocation: MUNICH,
    homeRailItemIds: ['rail-1', 'rail-2', 'rail-3'],
    now: NOW,
    client,
  })
  assert.deepEqual(result.candidates, [], 'candidates correctly goes to 0 — all 3 eligible items are already on Home Rail')
  assert.equal(result.eligibleLocalCount, 3, 'eligibleLocalCount must still reflect the full eligible local inventory, not the post-exclusion candidate pool')
})

test('eligibleLocalCount, unlike candidates.length, is unaffected by Home Rail exclusion in the normal (non-zero) case too', async () => {
  const items = [makeItem('rail-1'), makeItem('good-1'), makeItem('good-2')]
  const { client } = makeStubClient({ items, momentumRows: [] })
  const result = await assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: LOCATION, homeRailItemIds: ['rail-1'], now: NOW, client })
  assert.equal(result.candidates.length, 2, 'candidates excludes the Home Rail item, as before')
  assert.equal(result.eligibleLocalCount, 3, 'eligibleLocalCount counts all 3 eligible items, including the one on Home Rail')
})

test('eligibleLocalCount respects the same radius cutoff as candidates (Munich/Vienna)', async () => {
  const MUNICH = { latitude: 48.1351, longitude: 11.5820 }
  const items = [
    makeItem('strudlhofstiege', { lat: 48.21, lng: 16.37 }), // Vienna, ~355mi away
    makeItem('marienplatz', { lat: 48.1374, lng: 11.5755 }), // Munich
  ]
  const { client } = makeStubClient({ items, momentumRows: [] })
  const result = await assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: MUNICH, homeRailItemIds: [], now: NOW, client })
  assert.equal(result.eligibleLocalCount, 1, 'the out-of-radius Vienna item must not inflate eligibleLocalCount')
})

test('eligibleLocalCount ignores currentMetroId entirely — accepted for forward-compatibility only, no filtering behavior (see the METRO IDENTITY note in whatsGoodDataAdapter.js)', async () => {
  const items = [
    makeItem('a', { lat: 40.0, lng: -105.0 }),
    makeItem('b', { lat: 40.001, lng: -105.001 }),
  ]
  const { client } = makeStubClient({ items, momentumRows: [] })
  const withoutMetroId = await assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: LOCATION, homeRailItemIds: [], now: NOW, client })
  const withMetroId = await assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: LOCATION, homeRailItemIds: [], now: NOW, client, currentMetroId: 'any-metro-id' })
  assert.equal(withoutMetroId.eligibleLocalCount, 2)
  assert.equal(withMetroId.eligibleLocalCount, 2, 'currentMetroId is a no-op today — passing it must not change the result')
})

// KNOWN LIMITATION (documented, not fixed by this change — see the METRO
// IDENTITY note in whatsGoodDataAdapter.js): eligibleLocalCount is
// radius-only (MAX_NEARBY_RADIUS_M), with no per-metro identity check. Two
// metros within 100mi of each other (e.g. a Denver/Colorado-Springs-style
// pair) CAN currently leak into each other's eligibleLocalCount if their
// items fall within the same radius window. This test documents the CURRENT
// behavior (not the desired end state) so a future fix has a regression
// baseline to change deliberately, not accidentally.
test('KNOWN LIMITATION: an item belonging to a different, nearby metro (within MAX_NEARBY_RADIUS_M) currently DOES count toward eligibleLocalCount — no per-metro identity enforcement yet, radius is the only gate', async () => {
  // Colorado Springs is ~70mi from Denver — well within the 100mi radius cap.
  const DENVER = { latitude: 39.7392, longitude: -104.9903 }
  const items = [
    makeItem('denver-item', { lat: 39.74, lng: -104.99 }),
    makeItem('colorado-springs-item', { lat: 38.8339, lng: -104.8214 }),
  ]
  const { client } = makeStubClient({ items, momentumRows: [] })
  const result = await assembleWhatsGoodCandidates({
    userId: USER_ID,
    ...
    now: NOW,
    client,
    currentMetroId: 'denver-metro-id',
  })
  assert.equal(result.eligibleLocalCount, 2, 'KNOWN LIMITATION: the Colorado Springs item currently counts toward Denver eligibleLocalCount — radius-only, no metro-identity gate (flagged follow-up, not fixed here)')
})

test('items query error propagates as a rejection, never silently becomes eligibleLocalCount: 0', async () => {
  const { client } = makeStubClient({ itemsError: new Error('items query failed') })
  await assert.rejects(
    () => assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: LOCATION, homeRailItemIds: [], now: NOW, client }),
    /items query failed/
...
// ---------------------------------------------------------------------------
// 2026-09-23 FIELD BUG FIX (real root cause, not the two earlier fixes to
// this pipeline's derivation/cache layers): assembleWhatsGoodCandidates()'s
// items query had no .order()/.range() — PostgREST silently caps an
// unbounded .select() at 1000 rows. lib/supabasePagination.js's own header
// documents this EXACT bug already happening and being fixed elsewhere in
// this codebase (the "Vienna MORGEN bug, 2026-09-13", global eligible-items
// count confirmed at 1465 for this identical filter) — this function never
// adopted fetchAllRows(), so any item past row ~1000 in whatever order
// Postgres returned them was silently absent, BEFORE distance/eligibility
// was ever computed. No existing fixture in this file (a handful of items
// each) could ever exercise this — this is why a large fixture, not another
// hand-shaped one, is required.
//
// The stub below deliberately simulates PostgREST's real behavior: calling
// `.then()` directly (i.e. the code under test never calls `.range()`)
// truncates to the first 1000 rows, exactly like the real unbounded-select
// bug did. Calling `.range(from, to)` (i.e. fetchAllRows() is being used,
// as the fix now does) returns the correctly sliced page every time,
// eventually yielding every row. This test would FAIL against the
// pre-2026-09-23 code (which never called .range()) and PASSES only
// because assembleWhatsGoodCandidates now does.
// ---------------------------------------------------------------------------

function makeTruncatingItemsStubClient({ items, momentumRows = [] }) {
  function itemsHandler() {
    const handler = {
      ...
      order: () => handler,
      range: (from, to) => ({
        then: (resolve, reject) => Promise.resolve({ data: items.slice(from, to + 1), error: null }).then(resolve, reject),
      }),
      // Simulates PostgREST's real silent 1000-row default cap for a query
      // that never calls .range() at all — the exact pre-fix shape.
      then: (resolve, reject) => Promise.resolve({ data: items.slice(0, 1000), error: null }).then(resolve, reject),
    }
    return handler
  }
  return {
    from(table) {
      if (table === 'items') return itemsHandler()
      if (table === 'check_ins') return { select: () => ({ eq: () => ({ in: () => Promise.resolve({ data: [], error: null }) }) }) }
      if (table === 'whats_good_exposures') return { select: () => ({ eq: () => ({ in: () => Promise.resolve({ data: [], error: null }) }) }) }
      if (table === 'list_items') return { select: () => ({ eq: () => Promise.resolve({ data: [], error: null }) }) }
...
test('REGRESSION (2026-09-23 field bug — the real root cause): a >1000-row eligible-local-inventory (e.g. the global catalog crossing PostgREST\'s default cap) must not silently truncate — a Munich item sitting past row 1000 must still be fetched and counted', () => {
  // 1049 items total, ALL genuinely within Munich's radius (matching real
  // production shape — Munich's real catalog has hundreds of items, not
  // one) so eligibleLocalCount only comes out correct if every row was
  // actually fetched. 1048 filler rows fill exactly the truncated stub's
  // first-1000-row response on their own, plus ONE specifically-named
  // Munich item placed at the very end (index 1048, well past row 1000) to
  // prove it individually survives, not just the aggregate count.
  const items = []
  for (let i = 0; i < 1048; i++) {
    items.push(makeItem(`filler-${i}`, { lat: 48.13 + i * 0.00001, lng: 11.57 })) // near Munich, tiny jitter, just bulk
  }
  items.push(makeItem('marienplatz-past-row-1000', { lat: 48.1374, lng: 11.5755 }))

  const client = makeTruncatingItemsStubClient({ items })
  return assembleWhatsGoodCandidates({
    userId: USER_ID,
    userLocation: { latitude: 48.1351, longitude: 11.5820 }, // Munich
    homeRailItemIds: [],
    now: NOW,
    client,
  }).then((result) => {
    assert.equal(result.eligibleLocalCount, 1049, 'every eligible row must be counted, including the one past PostgREST\'s 1000-row default cap — a truncated fetch would report 1000 here')
    assert.ok(
      result.candidates.some((c) => c.itemId === 'marienplatz-past-row-1000'),
      'the real Munich item sitting past row 1000 must still reach the candidate pool, not be silently dropped by an unpaginated query'
    )
  })
})

// ---------------------------------------------------------------------------
// ADMIN DIAGNOSTICS PANEL (Phase 1, 2026-09-23) — `diagnostics` is a purely
// additive return field; these tests prove its counts are arithmetically
// consistent with each other and with the pre-existing `eligibleLocalCount`/
// `candidates` fields, and that homeRailExcludedCount specifically counts
// Home-Rail-caused exclusions (not conflated with the CANDIDATE_POOL_SIZE
// cap).
// ---------------------------------------------------------------------------

test('diagnostics: homeRailExcludedCount matches the manually-computed count of sortedItems that are on the Home Rail, independent of the 15-item cap', async () => {
  // 20 eligible local items total; 3 are on the Home Rail (scattered, not
  // just the nearest ones) — the candidate pool is capped at 15, so
  // eligibleLocalCount (20) - candidates.length would wrongly read 5 if it
  // conflated "excluded for Home Rail" with "excluded for the cap." The
  // correct homeRailExcludedCount is exactly 3.
  const homeRailIds = new Set(['item-2', 'item-9', 'item-17'])
  const items = Array.from({ length: 20 }, (_, i) => makeItem(`item-${i}`, { lat: 40.0 + i * 0.001, lng: -105.0 }))
  const { client } = makeStubClient({ items, momentumRows: [] })
  const result = await assembleWhatsGoodCandidates({
    userId: USER_ID,
    userLocation: LOCATION,
    homeRailItemIds: [...homeRailIds],
    now: NOW,
    client,
  })

  assert.equal(result.eligibleLocalCount, 20)
  assert.equal(result.diagnostics.eligibleLocalCount, 20)
  assert.equal(result.diagnostics.homeRailExcludedCount, 3)
  assert.equal(result.candidates.length, 15, 'capped pool size, confirming the cap and Home Rail exclusion are two distinct effects')
  assert.equal(result.diagnostics.candidatePoolSize, 15)
  // Manual cross-check: eligibleLocalCount - candidates.length (5) must NOT
  // equal homeRailExcludedCount (3) in this fixture — proving the two counts
  // really are distinct, not accidentally always identical.
  assert.notEqual(result.eligibleLocalCount - result.candidates.length, result.diagnostics.homeRailExcludedCount)
})

test('diagnostics: rawItemCount, outOfSeasonOrNoCoordsCount, bonusDropMaskedCount, and coordinateEligibleLocalCount are internally consistent', async () => {
  const inSeasonEligible = [makeItem('a'), makeItem('b'), makeItem('c')]
  const outOfSeason = [makeItem('out-of-season-item', { season_tag: OUT_OF_SEASON_TAG })]
  const items = [...inSeasonEligible, ...outOfSeason]
  const { client } = makeStubClient({ items, momentumRows: [], bonusDropItemIds: ['b'] }) // 'b' is a locked bonus drop -> masked
  const result = await assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: LOCATION, homeRailItemIds: [], now: NOW, client })

  assert.equal(result.diagnostics.rawItemCount, items.length)
  assert.equal(result.diagnostics.rawItemCount, result.diagnostics.inSeasonAndCoordEligibleCount + result.diagnostics.outOfSeasonOrNoCoordsCount)
  assert.equal(result.diagnostics.inSeasonAndCoordEligibleCount, result.diagnostics.coordinateEligibleLocalCount + result.diagnostics.bonusDropMaskedCount)
  assert.equal(result.diagnostics.coordinateEligibleLocalCount, result.diagnostics.eligibleLocalCount, 'no maxDistance cutoff triggered in this fixture, so the two should agree')
})

test('diagnostics: an empty candidate pool (all Home Rail) still returns a fully-populated diagnostics object', async () => {
  const items = [makeItem('rail-only')]
  const { client } = makeStubClient({ items })
  const result = await assembleWhatsGoodCandidates({ userId: USER_ID, userLocation: LOCATION, homeRailItemIds: ['rail-only'], now: NOW, client })
  assert.deepEqual(result.candidates, [])
  assert.equal(result.diagnostics.rawItemCount, 1)
  assert.equal(result.diagnostics.eligibleLocalCount, 1)
  assert.equal(result.diagnostics.homeRailExcludedCount, 1)
  assert.equal(result.diagnostics.candidatePoolSize, 0)
})
