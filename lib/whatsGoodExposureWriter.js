...
export async function recordWhatsGoodExposure({ userId, itemIds, now, client }) {
  // Anonymous (no userId): whats_good_exposures.user_id is NOT NULL and its
  // RLS is self-row-only, so there is nothing valid to write — anon simply
  // gets no rotation memory (always a fresh, non-personalized selection),
  // same as a not-yet-loaded signed-in session.
  if (!userId) return
