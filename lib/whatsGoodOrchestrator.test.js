...
test('local-pool fallback fills a shortage from the SAME local candidate pool, never from Universal, even when Universal items are passed in allLocatedItems', async () => {
  // Only 1 local (non-universal) candidate available in the adapter's own
  // query result -> selector returns at most 1 -> the fallback must still
  // end up with just that 1 (there is nothing else in the local pool to
  // broaden into), and must NEVER reach into allLocatedItems' Universal
  // items to pad the count.
  ...
  assert.deepEqual(result.itemIds, ['local-1'], 'a genuine local-inventory shortage must show fewer cards, never Universal filler')
  assert.ok(result.itemIds.every((id) => !id.startsWith('universal-')), 'no Universal item may ever appear in the result')
})
...
test('a genuinely empty local pool (0 eligible candidates) produces an empty result, never a Universal-filled one', async () => {
