    ...
    getWhatsGoodSelection({
      ...
      .then(({ itemIds, fromCache, debug, backupItemIds }) => {
        ...
        if (verified.length < hydrated.length) {
          console.warn(
            `useWhatsGood: final geographic gate rejected ${hydrated.length - verified.length} of ${hydrated.length} hydrated item(s) as out-of-radius, coordinate-less, or Universal — showing ${verified.length} instead of backfilling from elsewhere.`
          )
        }

        // DEV-only structural assertion (2026-09-21 second follow-up fix) —
        // findWhatsGoodContractViolations is the exact same pure predicate
        // verifyNearbyForRender just filtered with, so this should always
        // be empty; it exists as a regression trip-wire (and is unit-tested
        // directly in whatsGoodDataAdapter.test.js) so a future change that
        // accidentally appends items AFTER verifyNearbyForRender, or
        // reintroduces a Universal-item leak, fails loudly in dev rather
        // than silently reaching production.
        if (typeof __DEV__ !== 'undefined' && __DEV__) {
          const violations = findWhatsGoodContractViolations(verified, userLocation)
