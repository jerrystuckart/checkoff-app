...
// The fallback of last resort, when there's no explicit prior selection AND
// no usable location (denied permission, or a genuinely failed/timed-out
// fetch — never "still loading"). Alphabetically-first active metro:
// metro-blind, not a hardcoded city name/coordinate. This intentionally
// replaces every hardcoded-Phoenix (or Phoenix-vs-Milwaukee-by-latitude)
// special case in this codebase.
...
//   4. Otherwise (location genuinely denied/unavailable, not merely
//      pending, and no coords on any metro), fall back to fallbackMetro().
//      This case is intentionally metro-blind, not a disguised "resolved
//      nearest metro" — callers should treat `reason` accordingly (e.g. not
//      persist it as though it were an explicit choice).
//
...
export function resolveHomeMetro({ persistedSlug, metros, location, locationState }) {
  ...
  return { metro: fallbackMetro(list), reason: 'fallback' }
}
