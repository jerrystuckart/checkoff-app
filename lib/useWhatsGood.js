...
export function useWhatsGood({ userId, rawNearbyItems, homeRailItemIds, navigation }) {
  ...
  // isFlagEnabled()'s own internal caching) + an initial location fix
  // (force:false — reuses a recent-enough fix another screen already has).
  useEffect(() => {
    if (!userId) {
      setEnabled(false)
    ...
    isFlagEnabled(userId, 'whats_good_v1').then(async (isEnabled) => {
      ...
      if (isEnabled) await refreshLocation(false)
    })
    return () => {
      cancelled = true
    }
