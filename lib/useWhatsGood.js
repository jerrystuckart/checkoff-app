...
export function useWhatsGood({ userId, rawNearbyItems, homeRailItemIds, currentMetroId = null, locationState: locationStateProp, explicitMetroChoice = null, navigation }) {
  ...
  // Phase 4 (2026-09-22) — surfaces the coverage mode the selection effect
  // below already computes (`effectiveCoverageMode`) so HomeScreen.jsx can
  // render an explicit "unsupported location" state instead of the prior
  // silent empty/unlabeled-Universal-items gap. No selection/filtering
  // logic changes — this is purely an additional exposed value.
  const [coverageMode, setCoverageMode] = useState(COVERAGE_MODE.PENDING_LOCATION)
  const [debugInfo, setDebugInfo] = useState({
    ...
    atPlaceItemId: null,
    // ADMIN DIAGNOSTICS PANEL (Phase 1, 2026-09-23) — additive-only, mirrors
    // whatever lib/whatsGoodOrchestrator.js's `debug` object now returns
    // (see getWhatsGoodSelection), plus this hook's own locationState.
    locationState: null,
  ...
  useEffect(() => {
    ...
    if (!userLocation) {
      ...
      setCoverageMode(locationState === 'pending' ? COVERAGE_MODE.PENDING_LOCATION : COVERAGE_MODE.LOCATION_UNAVAILABLE)
      return undefined
    }
    ...
    getWhatsGoodSelection({
      ...
      .then(({ itemIds, fromCache, debug, backupItemIds, coverageMode }) => {
        ...
        setCoverageMode(effectiveCoverageMode)
        ...
        setDebugInfo({
          ...
          // threaded straight through from the orchestrator's `debug`
          // object (see lib/whatsGoodOrchestrator.js), plus this hook's own
          // already-computed `locationState`.
          locationState,
          cachedSchemaVersion: debug?.cachedSchemaVersion ?? null,
          cachedCoverageMode: debug?.cachedCoverageMode ?? null,
          preservationDecision: debug?.preservationDecision ?? null,
          diagnostics: debug?.diagnostics ?? null,
          universalCount: debug?.universalCount ?? null,
        })
      })
      .catch((e) => {
  ...
  return { enabled, loading, items: selectedItems, atPlaceItem, coverageMode, debug: debugInfo }
}
