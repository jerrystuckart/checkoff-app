...
// item, restricted to lists this user is already a member of. A check-off
// must never change list memberships (see lib/joinListCredit.js for the
// converse: crediting a list once the user actually joins it) — this only
// mirrors into lists the user already intentionally belongs to.
async function getMemberListItemsForItem(itemId, userId) {
 ...
 * has already, intentionally joined. points_awarded is omitted on every
 * fanned-out row so a single real action is never double-counted in
 * lifetime points. Uses upsert + ignoreDuplicates so it's safe to call
 * even when some target lists already have a row for this item.
 * Non-critical — never throws.
 *
 * Does NOT join the user to any list — checking off an item must not
 * change list memberships (product decision, 2026-08). Only mirrors into
 * lists the user already belongs to; a list the item also happens to sit
 * on, that the user hasn't joined, is left alone.
 */
export async function fanOutCheckIn({ userId, itemId, excludeListItemId = null, checkinMethod = 'tap', photoUrl = null }) {
  if (!itemId || !userId) return
  try {
    const memberListItems = await getMemberListItemsForItem(itemId, userId)

    const targets = memberListItems.filter(li => li.id !== excludeListItemId)
