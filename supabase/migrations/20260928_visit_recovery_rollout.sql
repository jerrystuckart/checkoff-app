-- =============================================================================
-- Visit recovery rollout foundations (2026-09-28).
--
-- Seven-day visit recovery is built so that it can be offered to everyone
-- without anyone being tracked who has not chosen it:
--
-- 1. visit_recovery_settings — one row per user, explicit opt-in. A new table
--    rather than a users column: public.users has column-level grants (clients
--    may UPDATE only app_version/avatar_url), so a users column would need an
--    RPC just to be read or written.
-- 2. candidate_visits INSERT now also requires the user's own opt-in row. The
--    server therefore refuses to retain visit data for anyone who has not
--    opted in, regardless of client version or feature-flag state.
-- 3. turn_off_visit_recovery() — one call that opts the user out AND deletes
--    their retained visit data (candidate visits, geofence registration log,
--    geofence debug events). Confirmed check-ins are the user's real check-offs
--    and are deliberately NOT touched.
-- 4. cleanup_visit_recovery_data() + a daily job — retention: unconfirmed
--    candidates are removed 1 day after their 7-day window closes; confirmed
--    candidates (only a link to the check-in) after 30 days; diagnostics after
--    14 days. Nothing else in this migration schedules or deletes anything.
-- 5. The tester (visit_detection_tester) is seeded as opted in: their phone is
--    already recording candidates under the previous rules.
--
-- Unchanged on purpose: departure pushes stay tester-only (the notify trigger
-- still requires users.visit_detection_tester), the confirm authorization in
-- prevent_expired_list_checkins() (ownership, item match, status, 7-day expiry,
-- duplicate guards) has no tester condition and is untouched, and the ordinary
-- at-place check-off path is untouched.
-- =============================================================================

BEGIN;

CREATE TABLE IF NOT EXISTS public.visit_recovery_settings (
...
ALTER TABLE public.visit_recovery_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS visit_recovery_settings_select_own ON public.visit_recovery_settings;
CREATE POLICY visit_recovery_settings_select_own ON public.visit_recovery_settings
  FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS visit_recovery_settings_insert_own ON public.visit_recovery_settings;
CREATE POLICY visit_recovery_settings_insert_own ON public.visit_recovery_settings
  FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS visit_recovery_settings_update_own ON public.visit_recovery_settings;
CREATE POLICY visit_recovery_settings_update_own ON public.visit_recovery_settings
  FOR UPDATE USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

REVOKE ALL ON public.visit_recovery_settings FROM anon;
GRANT SELECT, INSERT, UPDATE ON public.visit_recovery_settings TO authenticated;

-- ---------------------------------------------------------------------------
-- candidate_visits INSERT: same plausibility rules as 20260923 plus opt-in.
-- ---------------------------------------------------------------------------
DROP POLICY IF EXISTS candidate_visits_insert_own ON public.candidate_visits;
CREATE POLICY candidate_visits_insert_own ON public.candidate_visits
  FOR INSERT WITH CHECK (
    user_id = auth.uid()
    AND EXISTS (SELECT 1 FROM public.visit_recovery_settings s WHERE s.user_id = auth.uid() AND s.opted_in)
...
-- ---------------------------------------------------------------------------
-- FIX to 20260927: the "confirmed rows cannot be relabelled" guard also blocked
-- the FK's own ON DELETE SET NULL update, so deleting a confirmed candidate
-- visit (turn-off, retention) failed. Found by this migration's rollback test.
-- Only the guard's condition changes; everything else is byte-for-byte 20260927.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Detaching the candidate link (matched_candidate_visit_id -> NULL with the
  -- label unchanged) is allowed: it is what the FK's ON DELETE SET NULL does
  -- when a candidate visit is deleted (user turn-off, retention job), and it
  -- cannot create or strip a confirmation.
  IF TG_OP = 'UPDATE'
     AND (NEW.verification_method IS DISTINCT FROM OLD.verification_method
          OR (NEW.matched_candidate_visit_id IS DISTINCT FROM OLD.matched_candidate_visit_id
              AND NEW.matched_candidate_visit_id IS NOT NULL))
...
-- ---------------------------------------------------------------------------
-- Turn off + delete retained visit data.
-- ---------------------------------------------------------------------------
...
DECLARE
  v_uid uuid := auth.uid();
  v_cv int; v_reg int; v_dbg int;
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'Not signed in.' USING ERRCODE = 'P0001';
  END IF;

  INSERT INTO visit_recovery_settings (user_id, opted_in, opted_in_at, updated_at)
  ...
  RETURN jsonb_build_object('candidate_visits', v_cv, 'registration_log', v_reg, 'debug_events', v_dbg);
END;
$$;

REVOKE ALL ON FUNCTION public.turn_off_visit_recovery() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.turn_off_visit_recovery() TO authenticated;
...
SET search_path TO 'public'
AS $$
DECLARE v_open int; v_confirmed int; v_reg int; v_dbg int;
BEGIN
  DELETE FROM candidate_visits
   WHERE status <> 'confirmed' AND expires_at < now() - interval '1 day';
  GET DIAGNOSTICS v_open = ROW_COUNT;

  DELETE FROM candidate_visits
   WHERE status = 'confirmed' AND COALESCE(confirmed_at, created_at) < now() - interval '30 days';
  ...
  RETURN jsonb_build_object('unconfirmed', v_open, 'confirmed', v_confirmed, 'registration_log', v_reg, 'debug_events', v_dbg);
END;
$$;
...
DO $$
BEGIN
  PERFORM cron.unschedule('cleanup-visit-recovery-data')
  WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'cleanup-visit-recovery-data');
  PERFORM cron.schedule('cleanup-visit-recovery-data', '20 3 * * *', 'SELECT public.cleanup_visit_recovery_data()');
END $$;

-- ---------------------------------------------------------------------------
-- The tester's phone is already recording under the previous rules.
-- ---------------------------------------------------------------------------
INSERT INTO visit_recovery_settings (user_id, opted_in, opted_in_at)
SELECT id, true, now() FROM users WHERE visit_detection_tester
ON CONFLICT (user_id) DO NOTHING;

DO $$
BEGIN
  IF (SELECT count(*) FROM cron.job WHERE jobname = 'cleanup-visit-recovery-data') <> 1 THEN
    RAISE EXCEPTION 'retention job was not scheduled';
  END IF;
  IF (SELECT count(*) FROM visit_recovery_settings WHERE opted_in) <> (SELECT count(*) FROM users WHERE visit_detection_tester) THEN
    RAISE EXCEPTION 'unexpected opted-in rows: only the tester should be seeded';
  END IF;
END $$;
