  ...
  async function load() {
    ...
    try {
      let query = supabase
        ...
        .eq('active', true)
        .in('distance_type', ['day-trip', 'weekend'])
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Places worth the drive from {metroLabel}</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptySub}>We're adding day trips and weekend spots for {metroLabel}. Check back soon.</Text>
        </View>
      ) : (
        <ScrollView
...
function DestinationCard({ item, onPress, styles, colors }) {
  ...
  const cardInner = (
    <View style={styles.cardBody}>
      ...
      <Text style={[styles.cardTitle, !item.image_url && { color: colors?.TEXT ?? '#FFFFFF' }]} numberOfLines={2}>{item.title}</Text>
      {!!item.subtitle && (
        <Text style={[styles.cardSubtitle, !item.image_url && { color: colors?.MUTED ?? 'rgba(255,255,255,0.85)' }]} numberOfLines={2}>{item.subtitle}</Text>
      )}

      <View style={styles.cardFooter}>
        <Text style={[styles.cardCity, !item.image_url && { color: colors?.MUTED ?? 'rgba(255,255,255,0.7)' }]}>{item.city ?? ''}</Text>
