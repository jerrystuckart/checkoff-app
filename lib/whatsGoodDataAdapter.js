...
/**
 * FINAL, UNCONDITIONAL geographic/eligibility render gate (2026-09-21
 * follow-up field bug fix; tightened 2026-09-21 second follow-up — see
 * decision `whats_good_v1_never_universal`). Applied by the caller
 * (lib/useWhatsGood.js) to the fully hydrated item objects for the
 * selection ABOUT TO BE RENDERED — regardless of whether those item ids
 * came from a fresh candidate query, the session cache, or the local-pool
 * fallback broadening (lib/whatsGoodOrchestrator.js). Nothing upstream of
 * this point (cache preservation, ranking, momentum/personalization score,
 * tag relevance, any fallback branch) is trusted to have already enforced
 * geography/eligibility correctly; this function re-verifies it
 * independently, one more time, right before render, so a bug anywhere
 * upstream can never put an out-of-radius OR a Universal (non-place-based)
 * card on screen.
 *
 * Rules (see isEligibleForWhatsGood below for the single source of truth):
 *   - A Universal item (`is_universal`/`isUniversal` true) is NEVER
 *     eligible for What's Good, under any fallback tier — What's Good is
 *     place-based by definition (it exists to surface a real nearby venue,
 *     unlike the Home Rail's own separate, deliberate Universal interleave
 *     in lib/proximity.js, which this function has no bearing on). This
 *     REVERSES this function's original exemption, which treated a
 *     Universal item as "not a location-based card, so nothing to verify,
 *     always passes" — that exemption was the actual leak: it let the
 *     orchestrator's now-removed Universal top-up fallback sail straight
 *     through what was supposed to be the unconditional final gate.
 *   - Every item MUST have usable coordinates (lib/nearbyRanking.js's
 ...
 *     another city (or from Universal) to hit a target count (that's the
 *     caller's job to accept: fewer cards, or an honest empty state).
 *   - If `userLocation` itself isn't usable, every item is rejected (there
 *     is nothing to verify distance against, and Universal items are no
 *     longer exempt from needing a real place to verify).
 *
...
export function verifyNearbyForRender(items, userLocation, options = {}) {
  return (items ?? []).filter((item) => isEligibleForWhatsGood(item, userLocation, options))
}

/**
 * Single-item version of the same contract verifyNearbyForRender applies to
 * a list — pure predicate, no I/O. Exists as its own export (rather than
 * only inlined inside verifyNearbyForRender's filter) specifically so:
...
export function isEligibleForWhatsGood(item, userLocation, { maxDistance = MAX_NEARBY_RADIUS_M } = {}) {
  ...
  if (isUniversal) return false // What's Good is place-based only — Universal items are never eligible, under any fallback tier
 ...
 * pure-logic-extraction convention, rather than an inline console.warn
 * that could only ever be exercised manually. Returns the ids of any item
 * in `items` that does NOT satisfy the full What's Good place-based
 * geographic contract (active/coordinate-bearing/in-radius/non-Universal —
 * "active" itself is enforced upstream by assembleWhatsGoodCandidates'
 * own `is_active` query filter, not re-checked here since this function
 * only ever sees already-selected item objects that don't carry that
 * field). An empty array means the contract holds for every item.
 *
 * @param {Array} items
 * @param {{latitude:number, longitude:number}|null} userLocation
 * @param {{maxDistance?: number}} [options]
...
export function findWhatsGoodContractViolations(items, userLocation, options = {}) {
  return (items ?? [])
    .filter((item) => !isEligibleForWhatsGood(item, userLocation, options))
    .map((item) => item?.id)
}
