...
async function reportEnter(itemId) {
  ...
  if (!fix) { await logDebugEventForCurrentUser('presence_rpc_failed', itemId, { stage: 'enter', message: 'no location fix' }); return }
...
async function flushPendingPresence() {
  ...
  const remaining = []
  for (const e of list) {
    try { if (e.type === 'enter') await sendEnter(e); else await sendExit(e) } catch { remaining.push(e) }
  }
...
async function clearArrivalRecords() {
  try {
    const keys = (await AsyncStorage.getAllKeys()).filter(k => k.startsWith(ARRIVAL_KEY_PREFIX) || k === PENDING_KEY)
