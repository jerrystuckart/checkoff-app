...
test('shared hierarchy: title, points metadata, primary action all present', () => {
  // Right Here Hero redesign (Phase 2/5, 2026-09-22) — "What's the Thing?" /
  // "Check it off" were replaced by "HERE'S THE THING" / "I DID THE THING"
  // per the activation-project spec (statement framing, not a question;
  // "the thing to do", never "checking in").
  assert.ok(heroSource.includes('HERE\'S THE THING'), 'title copy must still exist')
  assert.ok(heroSource.includes('I DID THE THING'), 'primary action copy must still exist')
