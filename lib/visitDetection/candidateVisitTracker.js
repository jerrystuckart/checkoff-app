...
TaskManager.defineTask(GEOFENCE_TASK_NAME, async ({ data, error }) => {
  ...
  try {
    if (eventType === Location.GeofencingEventType.Enter) {
      // The SERVER starts the visit clock (visit_presence_enter stamps entered_at with its own
      // time and keeps the original stamp when iOS re-delivers "enter" on every geofence
      // re-registration). The phone only reports that it entered, with a location fix.
      await flushPendingPresence()
      if (!shouldSendEnter(await readOpen(), region.identifier, Date.now())) return // re-delivered enter for a session we already reported
      await logDebugEventForCurrentUser('enter', region.identifier, {})
      await reportEnter(region.identifier)
      return
    }
...
// [entered_at, now].
// ---------------------------------------------------------------------------
const PENDING_KEY = 'visitPresencePending'
const OPEN_KEY = 'visitPresenceOpen'
const REGISTERED_AT_KEY = 'visitGeofenceRegisteredAt'
...
async function handleDeparture(itemId) {
  const registeredAt = Number((await AsyncStorage.getItem(REGISTERED_AT_KEY)) ?? 0) || null
  const decision = decideExit({ openMap: await readOpen(), itemId, registeredAtMs: registeredAt, nowMs: Date.now() })
  if (decision === 'skip') return
  if (decision === 'reconcile') { await reconcileNow(); return }
...
async function clearArrivalRecords() {
  try {
    const keys = (await AsyncStorage.getAllKeys()).filter(k => k.startsWith(ARRIVAL_KEY_PREFIX) || k === PENDING_KEY || k === OPEN_KEY || k === REGISTERED_AT_KEY)
