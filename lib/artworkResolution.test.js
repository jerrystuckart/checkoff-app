...
// Hardening pass (2026-09-17) — Approach A: lib/fallbackArtSource.js now
// only returns a url for keys marked 'available', and ALL 23 v1 keys
// default to 'pending' (no real assets exist yet — see
// docs/fallback-art-manifest.md). The tests below that assert an
// 'archetype' tier are testing the pure KEY-SELECTION logic (which key
// would apply), a genuinely different concern from asset availability —
// so they temporarily flip the one key under test to 'available' and
// restore it afterwards, rather than duplicating a second always-available
// test registry. This module's own gating logic (`if (archetype?.url)`)
// needed no code change for Approach A — see the two dedicated Approach A
// tests at the bottom of this file, which exercise the REAL, all-pending
// registry with no override at all.
...
// ── Approach A, exercised against the REAL (all-pending) registry ───────

test('Approach A: a matched archetype key that is still pending (the real, current registry state) resolves to generic, not archetype', () => {
  const item = { fallback_art_key: 'hidden_entrance', categoryName: 'Adventure' }
  const result = resolveArtworkTier(item)
  assert.equal(result.tier, 'generic')
  assert.equal(result.url, null)
...
test('Approach A: no field in the real registry today can produce an archetype tier (every key is pending)', () => {
  const item = { categoryName: 'Food & drink' }
  const result = resolveArtworkTier(item)
  assert.equal(result.tier, 'generic')
})
