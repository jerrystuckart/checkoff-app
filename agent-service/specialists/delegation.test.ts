...
test('validateResultEnvelope: valid envelope with all required evidence passes', () => {
  const request = buildDelegationRequest('metro_builder', 'metro_launch', 'M4_COVERAGE_AUDIT', 'count categories', {}, ['candidateCount'])
...
test('validateResultEnvelope: missing required evidence key fails', () => {
  const request = buildDelegationRequest('metro_builder', 'metro_launch', 'M4_COVERAGE_AUDIT', 'count categories', {}, ['candidateCount', 'neighborhoodCounts'])
  const result = validateResultEnvelope(request, envelope())
...
test('validateResultEnvelope: an empty-string or empty-array evidence value counts as missing, never a fabricated pass', () => {
  const request = buildDelegationRequest('research_verifier', 'metro_launch', 'M6', 'verify', {}, ['closureFindings'])
...
test('validateResultEnvelope: jerryRequired=true with no jerryReason is invalid — Chief never escalates without a stated reason', () => {
  const request = buildDelegationRequest('destination_strategist', 'destination_hub_lifecycle', 'D2_DVA1', 'screen', {}, [])
  const result = validateResultEnvelope(request, envelope({ jerryRequired: true, jerryReason: null }))
  assert.equal(result.valid, false)
...
test('validateResultEnvelope: jerryRequired=true with a real reason is valid', () => {
  const request = buildDelegationRequest('destination_strategist', 'destination_hub_lifecycle', 'D2_DVA1', 'screen', {}, [])
  const result = validateResultEnvelope(request, envelope({ jerryRequired: true, jerryReason: 'DVA-1 never auto-advances to DVA-2' }))
  assert.equal(result.valid, true)
})

test('validateResultEnvelope: jerryReason set while jerryRequired is false is an inconsistent envelope, rejected', () => {
  const request = buildDelegationRequest('metro_builder', 'metro_launch', 'M4', 'audit', {}, [])
...
test('buildDelegationRequest: idempotent — same inputs produce a structurally identical request', () => {
  const a = buildDelegationRequest('metro_builder', 'metro_launch', 'M3_BROAD_DISCOVERY', 'research food category', { metro: 'San Diego' }, ['candidates'])
  const b = buildDelegationRequest('metro_builder', 'metro_launch', 'M3_BROAD_DISCOVERY', 'research food category', { metro: 'San Diego' }, ['candidates'])
