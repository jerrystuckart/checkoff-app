...
test('"Help the next person" support copy is defined exactly once in live code (design correction — mockup duplicated it)', () => {
  const occurrences = codeOnlySource.split('Help the next person').length - 1
  assert.equal(occurrences, 1, 'the "Help the next person" copy must be defined in exactly one place in source, not duplicated per branch')
})

test('Add a Photo helper text is gated on the same showContributionCTA eligibility as the Add a Photo action itself', () => {
  assert.ok(heroSource.includes('if (!showContributionCTA) return null'), 'helper text must reuse the existing showContributionCTA eligibility, not a new/duplicate condition')
})
