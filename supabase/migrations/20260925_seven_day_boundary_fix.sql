-- =============================================================================
-- Seven-day boundary consistency fix (2026-09-25).
--
-- FOUND DURING THIS SESSION'S SERVER-SIDE BOUNDARY TEST (rollback-only,
-- against production): the confirm authorization trigger's expiry check
-- used strict `<` (v_cv_expires_at < now()), which means a confirm attempt
-- landing at the EXACT expires_at instant was still allowed -- not rejected.
-- lib/visitDetection/candidateVisitConfirmation.js's isCandidateVisitSuggestible()
-- (the client-side display mirror) already used `<=` (hides a suggestion
-- exactly at its expiry instant). This tightens the server to match: `<=`,
-- so "confirmation just before seven days" still succeeds and "rejection at
-- seven days" is now literally true on both sides, not just in practice
-- (an exact-microsecond real-world collision was already vanishingly
-- unlikely, but the two layers should agree regardless).
--
-- Nothing else in this function changes -- reproduced byte-for-byte from
-- 20260924_visit_detection_duplicate_guard.sql otherwise.
-- =============================================================================
...
DO $$
BEGIN
  IF (SELECT prosrc FROM pg_proc WHERE proname = 'prevent_expired_list_checkins') NOT ILIKE '%v_cv_expires_at <= now()%' THEN
    RAISE EXCEPTION 'expected <= expiry check not found in live function definition';
  END IF;
END $$;
