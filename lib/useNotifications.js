...
import { openVisitInbox } from './visitDetection/inboxNavigation'
...
// Visit Detection Stage 2 (2026-09-24) — deep-links a tapped
// candidate_visit_high_confidence push straight to the specific suggestion
// (see supabase/functions/process-notification-queue/index.ts's
// buildMessage() case, which already set this data shape but had nothing
// downstream that read it — this was the missing link in the
// trigger -> queue -> sender -> tap chain). navigationRef may not be ready
// yet if the app just cold-started from the tap, so this retries briefly
// rather than silently dropping the navigation.
function navigateToVisitInbox(navigationRef, candidateVisitId) {
  let attempts = 0
  const tryNavigate = () => {
    if (navigationRef?.current?.isReady?.()) {
      openVisitInbox(navigationRef.current, candidateVisitId)
      return
    }
    attempts += 1
    if (attempts < 10) setTimeout(tryNavigate, 300)
  }
  tryNavigate()
}
 ...
 * @param {object} [navigationRef]  React Navigation's useNavigationContainerRef() result, used only to deep-link a candidate_visit_high_confidence tap to VisitInbox.
 */
export function useNotifications(userId, navigationRef) {
  ...
  useEffect(() => {
    ...
    responseListener.current = Notifications.addNotificationResponseReceivedListener(response => {
      ...
      if (data?.kind === 'candidate_visit_high_confidence') {
        navigateToVisitInbox(navigationRef, data.candidate_visit_id)
        return
      }
      if (data?.url) {
