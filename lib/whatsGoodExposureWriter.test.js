...
// ---------------------------------------------------------------------------
// REGRESSION (Bug 1 — obsolete logged-out Home): anonymous (no userId) must
// never attempt to write whats_good_exposures — user_id is NOT NULL and RLS
// is self-row-only, so a null-userId upsert would fail. Anon simply gets no
// rotation memory rather than an error.
// ---------------------------------------------------------------------------

test('REGRESSION: anonymous (userId=null) -> no upsert attempted at all', async () => {
  const { client, upsertCalls } = makeStubClient()
  await recordWhatsGoodExposure({ userId: null, itemIds: ['a', 'b'], now: NOW, client })
  assert.equal(upsertCalls.length, 0)
})
