...
test('resolveHomeMetro: explicit persisted selection wins even with a different ready location', () => {
  // User explicitly picked Denver, but is currently physically near Vienna
  // (traveling) — the explicit choice must still win, it is not silently
  ...
  assert.equal(metro.slug, 'denver')
  assert.equal(reason, 'explicit')
})

// ── Test 5: no prior metro + unavailable location does NOT silently choose
// Phoenix specifically — it uses the documented metro-blind fallback. ─────
test('resolveHomeMetro: no persisted choice + unavailable location falls back to the metro-blind default, not a hardcoded Phoenix', () => {
  const { metro, reason } = resolveHomeMetro({
    persistedSlug: null,
    metros: METROS,
    location: null,
  ...
  assert.equal(metro, fallbackMetro(METROS))
})

test('fallbackMetro is alphabetically-first-active-metro, not name- or coordinate-keyed to Phoenix', () => {
...
test('resolveHomeMetro never hardcodes a Phoenix/Milwaukee latitude-threshold special case for metros lacking coordinates', () => {
  ...
  // nearest-by-distance calc can't run, so this must hit the metro-blind
  // fallback (first in list = Milwaukee here) rather than Phoenix.
  const { metro, reason } = resolveHomeMetro({
    persistedSlug: null,
  ...
  assert.equal(reason, 'fallback')
  assert.equal(metro.slug, 'milwaukee')
})

// ── Test 6: an explicitly selected metro is not immediately overwritten ───
test('resolveHomeMetro: repeated calls with an explicit persistedSlug always return it, regardless of location churn', () => {
  ...
  assert.equal(third.metro.slug, 'vienna')
})
