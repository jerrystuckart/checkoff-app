// End-to-end (pure) walk of the visit pipeline with real Florence catalog
...
test('tracker uses the pipeline (no second, drifting copy of the rules) and registers the venue-scale radius', () => {
  ...
  assert.ok(t.includes('evaluateDeparture(') && t.includes('resolveEnterArrival('))
  assert.ok(!/DEFAULT_GEOFENCE_RADIUS_M/.test(t), 'tracker must not fall back to the 500 m check-off radius')
  assert.ok(t.includes('candidate_insert_failed'), 'a rejected candidate insert must be logged, not reported as created')
})
