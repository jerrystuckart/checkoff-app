...
// Phoenix-local `date` strings (YYYY-MM-DD). Comparing them directly as
...
// "past ends_at" even hours before Phoenix midnight. Converting checkedAt
// to its Phoenix-local calendar date first mirrors the DB trigger
// prevent_expired_list_checkins(), which evaluates
// (now() AT TIME ZONE 'America/Phoenix')::date against these same columns.
function toPhoenixDateString(isoTimestamp) {
  return new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Phoenix' }).format(new Date(isoTimestamp))
}
...
export function isWithinWindow(checkedAt, startsAt, endsAt) {
  ...
  const checkedDate = toPhoenixDateString(checkedAt)
