...
import { loadWhatsGoodSession, saveWhatsGoodSession, shouldPreserveSession, describeSessionPreservationDecision } from './whatsGoodSessionCache.js'
...
export async function getWhatsGoodSelection({
  ...
  if (!forceRefresh) {
    ...
    // ADMIN DIAGNOSTICS PANEL (Phase 1, 2026-09-23) — a diagnostics-only
    // mirror of the real decision above, computed ALONGSIDE it (never
    // instead of it — shouldPreserveSession's own call above is still what
    // actually gates behavior). See whatsGoodSessionCache.js's module doc.
    const preservationDecision = describeSessionPreservationDecision(cached, { now, currentFingerprint, currentLocation: userLocation, currentMetroId, backgroundPreserveMs })

    if (cachePreserved && (cached.coverageMode === COVERAGE_MODE.SUPPORTED_SPARSE || cached.coverageMode === COVERAGE_MODE.UNSUPPORTED)) {
      ...
      // extended 2026-09-23 field bug fix to also cover UNSUPPORTED — see
      // this module's incident note below). SUPPORTED_SPARSE and
      // UNSUPPORTED are the two cached modes whose correctness can go
      // stale WITHOUT any location/metro change at all: more local
      // inventory can simply become available at the SAME spot, OR (the
      // 2026-09-23 case) a session was cached as UNSUPPORTED by a bug that
      // has since been fixed elsewhere in this pipeline, and nothing else
      // would ever invalidate that stale verdict — parts 0-3's location/
      // metro checks all still pass (same device, same city, same metro),
      // so a stale UNSUPPORTED cache would otherwise be replayed forever.
      // SUPPORTED_SUFFICIENT is not included here: its correctness can only
      // regress via a location/metro change, which parts 0-3 already catch
      // cheaply (no query) — paying for this recheck there would cost a
      // query on every single cache hit, for no behavioral gain. This is
      // the only branch that pays for a fresh candidate query on an
      // otherwise-valid cache hit, specifically to close these two gaps.
      //
      // 2026-09-23 INCIDENT: a real Munich device kept showing "we haven't
      // unlocked this city yet" and Universal-only What's Good even after
      // the eligibleLocalCount derivation fix (previous commit) shipped —
      // Near You and the resolved metro were both correct the whole time.
      // Root cause: that device's AsyncStorage already held a session
      // cached as UNSUPPORTED from BEFORE the derivation fix landed (when
      // eligibleLocalCount was still wrongly computed from the
      // Home-Rail-excluded personalization pool). The derivation fix only
      // changed what a FRESH computation produces — it did nothing to
      // invalidate an already-persisted stale verdict, and this function's
      // cache-hit fast path (`if (cachePreserved) return cachedResult(...)`
      // below) had no mechanism to ever recheck an UNSUPPORTED session.
      // Adding UNSUPPORTED to this drift-check closes that gap the same
      // way it was already closed for SUPPORTED_SPARSE.
      const { candidates: freshCandidates, eligibleLocalCount: freshEligibleLocalCount, diagnostics: freshDiagnostics } = await assembleWhatsGoodCandidates({ userId, userLocation, homeRailItemIds, now, client, currentMetroId })
      const { ranked: freshRanked } = selectWhatsGood(freshCandidates, now)
      const freshMode = deriveCoverageMode({ locationState, eligibleLocalCount: freshEligibleLocalCount, targetCount: WHATS_GOOD_TARGET_COUNT, hasExplicitSelection })
      if (freshMode === cached.coverageMode) {
        return cachedResult({ cached, myGeneration, preservationDecision, diagnostics: freshDiagnostics, allLocatedItems })
      }
      ...
      return composeAndPersist({
        ...
        eligibleLocalCount: freshEligibleLocalCount, diagnostics: freshDiagnostics, preservationDecision,
      })
    }

    if (cachePreserved) {
      return cachedResult({ cached, myGeneration, preservationDecision, allLocatedItems })
    }
  }
  ...
  const { candidates, eligibleLocalCount, diagnostics } = await assembleWhatsGoodCandidates({ userId, userLocation, homeRailItemIds, now, client, currentMetroId })
  const { selectedItemIds, ranked } = selectWhatsGood(candidates, now)
  return composeAndPersist({
    userId, userLocation, currentMetroId, homeRailItemIds, allLocatedItems, currentFingerprint, now, client, storage,
    locationState, hasExplicitSelection, myGeneration, candidates, ranked, selectedItemIds, eligibleLocalCount, diagnostics,
  })
}
...
// ADMIN DIAGNOSTICS PANEL (Phase 1, 2026-09-23) — counts the Universal item
// pool the caller already fetched (allLocatedItems — see this module's own
// param doc for fillUniversalTopUp's identical reuse of this same list). No
// new query; purely a length over data already in hand.
function countUniversalItems(allLocatedItems) {
  return (allLocatedItems ?? []).filter((item) => (item?.is_universal ?? item?.isUniversal ?? false)).length
}

function cachedResult({ cached, myGeneration, preservationDecision = null, diagnostics = null, allLocatedItems = null }) {
  return {
    ...
    debug: {
      ...
      coverageMode: cached.coverageMode,
      // ADMIN DIAGNOSTICS PANEL (Phase 1, 2026-09-23) — additive-only.
      cachedSchemaVersion: cached.schemaVersion ?? null,
      cachedCoverageMode: cached.coverageMode ?? null,
      preservationDecision: preservationDecision ?? null,
      diagnostics: diagnostics ?? null,
      universalCount: countUniversalItems(allLocatedItems),
    },
  }
}
 ...
 * fresh-computation path and the SUPPORTED_SPARSE/UNSUPPORTED cache-drift path above
 * (so neither one duplicates this logic or pays for an extra query).
 */
async function composeAndPersist({
  userId, userLocation, currentMetroId, homeRailItemIds, allLocatedItems, currentFingerprint, now, client, storage,
  locationState, hasExplicitSelection, myGeneration, candidates, ranked, selectedItemIds, eligibleLocalCount,
  diagnostics = null, preservationDecision = null,
}) {
  // eligibleLocalCount is the AUTHORITATIVE full-inventory count computed by
  // assembleWhatsGoodCandidates (2026-09-23 fix) — NOT ranked.length. ranked
  // is the Home-Rail-excluded personalization pool and continues to be used
  // exactly as before for actual item SELECTION (fillFromLocalPool,
  // backupItemIds, etc.) below — only the coverage-mode DERIVATION input
  // changed. See whatsGoodDataAdapter.js's module doc for the full incident.
  ...
  return {
    ...
    debug: {
      ...
      localFallbackUsed,
      coverageMode,
      // ADMIN DIAGNOSTICS PANEL (Phase 1, 2026-09-23) — additive-only, no
      // effect on selection/persistence above.
