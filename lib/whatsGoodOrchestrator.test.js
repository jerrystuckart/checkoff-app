...
// .range(from, to) actually slices result.data (array case) so tests can
// exercise lib/supabasePagination.js's fetchAllRows() with a genuinely
// multi-page fixture — see the >1000-row pagination regression test below.
...
function delayedChainable(result, delayMs) {
  const handler = {
    ...
    range: (from, to) => {
      ...
      return {
        then: (resolve, reject) => {
          setTimeout(() => Promise.resolve({ ...result, data: page }).then(resolve, reject), delayMs)
        },
      }
    },
    then: (resolve, reject) => {
    ...
    from(table) {
      if (table === 'items') {
        return {
          ...
          order: () => failingClient.from('items'),
          range: () => failingClient.from('items'),
...
// ---------------------------------------------------------------------------
// 2026-09-23 FIELD BUG FIX: the previous fix (deriving coverage mode from
// the adapter's authoritative eligibleLocalCount instead of ranked.length)
// only changed what a FRESH computation produces. A real Munich device
// still showed "we haven't unlocked this city yet" + Universal-only
// content, because its AsyncStorage already held a session CACHED as
// UNSUPPORTED from before that fix shipped — and the cache-hit fast path
// had no mechanism to ever recheck an UNSUPPORTED session (the drift check
// was wired for SUPPORTED_SPARSE only). Location, metro, and fingerprint
// all still matched (same device, same city), so the stale verdict was
// replayed forever. Fixed by extending the same drift-recheck to also
// cover UNSUPPORTED.
// ---------------------------------------------------------------------------

test('REGRESSION (2026-09-23 field bug — stale cached UNSUPPORTED session): resolved Munich metro, live Munich coordinates, Home Rail already contains real Munich items, additional eligible local inventory exists, but a STALE session cached as UNSUPPORTED (matching location/metro/fingerprint, well within the short-interruption window) must NOT be preserved once real inventory proves the city is supported', async () => {
  const MUNICH_LOC = { latitude: 48.1351, longitude: 11.5820 }
  const client = makeStubClient({
    items: [
      // Already shown on Home Rail (personalization/dedup excludes these
      // from `candidates`/`ranked` — must not affect coverage mode).
      makeItem('marienplatz', { maps_lat: 48.1374, maps_lng: 11.5755 }),
      makeItem('viktualienmarkt', { maps_lat: 48.1353, maps_lng: 11.5763 }),
      // NOT on Home Rail — genuinely available local inventory beyond the
      // rail, proving eligibleLocalCount reflects the full pool and that
      // real local items (not just a count) end up in the composition.
      makeItem('englischer-garten', { maps_lat: 48.1642, maps_lng: 11.6056 }),
      // A distant-metro item that must never be able to render regardless
      // of how the coverage-mode bug plays out.
      makeItem('strudlhofstiege-vienna', { maps_lat: 48.2211, maps_lng: 16.3600 }),
    ],
  })
  const allLocatedItems = [
    makeItem('marienplatz', { maps_lat: 48.1374, maps_lng: 11.5755 }),
    makeItem('viktualienmarkt', { maps_lat: 48.1353, maps_lng: 11.5763 }),
    makeItem('englischer-garten', { maps_lat: 48.1642, maps_lng: 11.6056 }),
    // Universal items ARE available in the pool the caller already has —
    // proving SUPPORTED_SUFFICIENT correctly ignores them rather than
    // padding with filler now that the real bug (a stale UNSUPPORTED
    // verdict) is fixed.
    makeItem('universal-1', { is_universal: true, maps_lat: null, maps_lng: null }),
    makeItem('universal-2', { is_universal: true, maps_lat: null, maps_lng: null }),
  ]
  // The exact stale-cache shape a real device would have persisted BEFORE
  // the eligibleLocalCount derivation fix: coverageMode UNSUPPORTED,
  // current schema, SAME location/metro/fingerprint the live call now
  // supplies, and well within the short-interruption preserve window —
  // every one of shouldPreserveSession's parts 0-4 would pass this as
  // compatible. Only the (now-extended) drift check can catch it.
  const staleCachedPayload = JSON.stringify({
    schemaVersion: 2,
    coverageMode: COVERAGE_MODE.UNSUPPORTED,
    itemIds: ['universal-1', 'universal-2'],
    generatedAt: new Date(NOW.getTime() - 30 * 1000).toISOString(),
    fingerprint: 'munich-fp',
    location: MUNICH_LOC,
    resolvedMetroId: 'munich-metro-id',
  ...
  const result = await getWhatsGoodSelection({
    ...
    userLocation: MUNICH_LOC,
    currentMetroId: 'munich-metro-id',
    homeRailItemIds: ['marienplatz', 'viktualienmarkt'],
    allLocatedItems,
    currentFingerprint: 'munich-fp', // deliberately matches the cached fingerprint too
    now: NOW,
    client,
    storage: makeStubStorage(staleCachedPayload),
  })

  assert.equal(result.fromCache, false, 'a stale UNSUPPORTED cache must be recomputed, not blindly replayed, once real inventory proves the city is supported')
  assert.equal(result.coverageMode, COVERAGE_MODE.SUPPORTED_SUFFICIENT, 'must never stay UNSUPPORTED once a fresh check proves real local inventory exists')
  assert.ok(!result.itemIds.includes('universal-1') && !result.itemIds.includes('universal-2'), 'the unsupported card / universal-only state must not survive: no Universal filler once genuinely SUPPORTED_SUFFICIENT')
  assert.ok(result.itemIds.includes('englischer-garten'), 'the rendered What\'s Good items must be real Munich-local places, not empty/fabricated content')
  assert.ok(!result.itemIds.includes('strudlhofstiege-vienna'), 'no Vienna/other-metro item can ever render, regardless of the coverage-mode bug')
  assert.ok(!result.itemIds.includes('marienplatz') && !result.itemIds.includes('viktualienmarkt'), 'Home Rail dedup still affects which items are SELECTED (personalization), even though it no longer affects coverage-mode CLASSIFICATION')
})
