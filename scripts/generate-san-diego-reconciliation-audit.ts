...
async function main() {
  ...
  push(`--`)
  push(`-- WHY THIS FILE EXISTS: agent_service (this repo's own DB role) cannot see the`)
  push(`-- real 141 existing staged San Diego/Tijuana items at all — \`items\` has RLS`)
  push(`-- enabled with a public-read policy gated on is_active=true AND is_approved=true,`)
  push(`-- and every one of the 141 real rows is is_active=false (correct, staged). That`)
  push(`-- silently filtered them out of every read this session ran, with no error to`)
  push(`-- signal it — confirmed via pg_policies/pg_roles, not guessed. So the exact`)
  push(`-- reconciliation counts below are UNVERIFIED until run under your own`)
  push(`-- privileged Supabase SQL Editor session, which is not subject to that filter.`)
  push(`--`)
  ...
  push(`    = (SELECT count(*) FROM _sd_matches) + (SELECT count(*) FROM _sd_obsolete)) AS proof_141_equals_retained_plus_obsolete,`)
  push(`  ((SELECT count(*) FROM _sd_final_candidates)`)
  push(`    = (SELECT count(*) FROM _sd_matches) + ((SELECT count(*) FROM _sd_final_candidates) - (SELECT count(*) FROM _sd_matches))) AS proof_152_equals_retained_plus_new;`)
  push(``)
