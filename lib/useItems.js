...
export function useItems(listId, showAlcohol = true) {
  ...
  const checkOff = useCallback(async (listItemId) => {
    ...
    if (wasChecked) {
      // Un-check — global by item_id, not just this list's row. A check-off
      // is a fact about the user and the item, so unchecking must be too:
