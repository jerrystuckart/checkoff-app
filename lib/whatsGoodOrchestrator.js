...
export async function getWhatsGoodSelection({
  ...
  if (!forceRefresh) {
    ...
    if (cachePreserved && (cached.coverageMode === COVERAGE_MODE.SUPPORTED_SPARSE || cached.coverageMode === COVERAGE_MODE.UNSUPPORTED)) {
      ...
      const { candidates: freshCandidates, eligibleLocalCount: freshEligibleLocalCount } = await assembleWhatsGoodCandidates({ userId, userLocation, homeRailItemIds, now, client, currentMetroId })
      ...
      return composeAndPersist({
        ...
        eligibleLocalCount: freshEligibleLocalCount,
      })
    }
  ...
  const { candidates, eligibleLocalCount } = await assembleWhatsGoodCandidates({ userId, userLocation, homeRailItemIds, now, client, currentMetroId })
