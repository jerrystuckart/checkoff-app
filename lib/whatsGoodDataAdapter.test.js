...
import { assembleWhatsGoodCandidates, verifyNearbyForRender } from './whatsGoodDataAdapter.js'
...
test('a Universal item passes the gate regardless of missing coordinates or distance — it is not a location-based card', () => {
  const items = [
    { id: 'universal-1', is_universal: true, maps_lat: null, maps_lng: null },
    { id: 'strudlhofstiege', maps_lat: VIENNA_COORDS.lat, maps_lng: VIENNA_COORDS.lng },
  ]
  const result = verifyNearbyForRender(items, MUNICH)
  assert.deepEqual(result.map((i) => i.id), ['universal-1'])
})

test('userLocation itself unusable -> every non-Universal item is rejected (nothing to verify distance against)', () => {
  const items = [
    { id: 'universal-1', is_universal: true },
    { id: 'marienplatz', maps_lat: 48.1374, maps_lng: 11.5755 },
  ]
  assert.deepEqual(verifyNearbyForRender(items, null).map((i) => i.id), ['universal-1'])
  assert.deepEqual(verifyNearbyForRender(items, {}).map((i) => i.id), ['universal-1'])
})
