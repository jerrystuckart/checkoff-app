...
// Item Detail Corrective Pass (2026-09-18), second pass — the old
// numberOfLines={2} clamp truncated real item bodies mid-word/mid-thought
// and is gone. Title presentation (length tier, font size, line budget,
// continuation split) is now owned by the pure lib/detailTitlePresentation.js
// helper — see lib/detailTitlePresentation.test.js for its own coverage —
// this screen-level test only guards that the old hard clamp doesn't come
// back and that the new helper is actually wired in.
test('long-title rule: no hard 2-line clamp remains; hero title is driven by deriveTitlePresentation', () => {
  ...
  assert.ok(source.includes('venueOverflowsHero'), 'the escape-hatch flag must exist and gate the compact continuation block')
})
...
// Item Detail Corrective Pass (2026-09-18), second pass — the below-hero
// continuation block (rendered only for titles that overflow the hero's
// 4-line budget) must not produce a SECOND, overlapping accessibility
// announcement of text already covered by heroContent's own grouped label
// (which always exposes the complete item.body exactly once).
test('below-hero continuation block is hidden from the accessibility tree (no duplicate exposure of already-announced body text)', () => {
  const wrapMatch = source.match(/style=\{styles\.heroContinuationWrap\}[\s\S]{0,120}/)
  assert.ok(wrapMatch, 'heroContinuationWrap render must exist')
  assert.ok(wrapMatch[0].includes('accessibilityElementsHidden'), 'the continuation block must be hidden from the accessibility tree')
  assert.ok(wrapMatch[0].includes('importantForAccessibility="no-hide-descendants"'), 'must use the same hide pattern already established by ArchetypeArtwork/DetailArtwork')
})
