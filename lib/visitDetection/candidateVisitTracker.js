...
import { loadConfidenceConfig } from './confidenceScore'
import {
  visitGeofenceRadiusM,
  findCompetingVenues,
  resolveEnterArrival,
  evaluateDeparture,
  isFixInsideVenue,
...
TaskManager.defineTask(GEOFENCE_TASK_NAME, async ({ data, error }) => {
  ...
  try {
    if (eventType === Location.GeofencingEventType.Enter) {
      // iOS re-delivers "enter" for regions we're already inside every time
      // the monitored set is re-registered (each foreground refresh), so an
      // existing recent arrival record is the REAL arrival and must not be
      // reset — otherwise dwell restarts on every refresh and no visit ever
      // reaches its threshold. See visitPipeline.resolveEnterArrival.
      const key = ARRIVAL_KEY_PREFIX + region.identifier
      const { arrivalMs, kept } = resolveEnterArrival(await AsyncStorage.getItem(key), Date.now())
      if (!kept) await AsyncStorage.setItem(key, String(arrivalMs))
      await logDebugEventForCurrentUser('enter', region.identifier, { arrivalKept: kept })
      return
    }
...
// Other visit-capable venues near this one, so an entry that could belong to
// any of several indistinguishable venues never produces a reminder. Bounding box
// of ~450 m covers any pair of visit radii (max 200 m each). Returns null on
// query failure (unknown — neither the competing nor the overlapping signal
// is set, rather than guessing).
async function loadNearbyVenueCandidates(item) {
  const d = 0.004
  const { data, error } = await supabase
    .from('items')
    .select('id, maps_lat, maps_lng, geo_radius_m, visit_profile_key, is_universal, is_active')
    .eq('is_active', true)
    .not('visit_profile_key', 'is', null)
    .gte('maps_lat', item.maps_lat - d).lte('maps_lat', item.maps_lat + d)
    .gte('maps_lng', item.maps_lng - d).lte('maps_lng', item.maps_lng + d)
  if (error || !data) return null
  return data
}

const DISCARD_EVENT = {
  manual_only_or_no_profile: 'discarded_manual_only_or_no_profile',
  below_candidate_dwell: 'discarded_below_candidate_dwell',
  implausible_dwell: 'discarded_implausible_dwell',
  negative_dwell: 'discarded_implausible_dwell',
  below_ignore_band: 'discarded_below_ignore_band',
}
...
async function handleDeparture(itemId) {
  ...
  const nearby = await loadNearbyVenueCandidates(item)
  const competingCount = nearby ? findCompetingVenues(item, nearby, profiles).length : null
  const { weights, bands } = await loadConfidenceConfig()

  const result = evaluateDeparture({
    userId,
    item,
    profile,
    arrivalMs: Number(arrivalRaw),
  ...
  if (result.outcome === 'discard') {
    await logDebugEventForCurrentUser(DISCARD_EVENT[result.reason] ?? 'discarded_below_ignore_band', itemId, {
      reason: result.reason,
      dwellMinutes: result.dwellMinutes != null ? Math.round(result.dwellMinutes * 10) / 10 : undefined,
      score: result.score,
    })
    return
  }

  const { error: insertError } = await supabase.from('candidate_visits').insert(result.row)
  if (insertError) {
    // Previously unchecked: an RLS/constraint rejection was logged as a
    // created candidate. Now visible in geofence_debug_events.
    await logDebugEventForCurrentUser('candidate_insert_failed', itemId, { message: insertError.message, score: result.score })
    return
  }

  await logDebugEventForCurrentUser('candidate_created', itemId, {
    dwellMinutes: result.row.dwell_minutes,
    confidenceScore: result.score,
    status: result.row.status,
    competingVenueCount: competingCount,
  })
}
...
async function clearArrivalRecords() {
  try {
    const keys = (await AsyncStorage.getAllKeys()).filter(k => k.startsWith(ARRIVAL_KEY_PREFIX))
...
// Marks candidate visits past their 7-day window (CANDIDATE_EXPIRY_MS in visitPipeline)
// as expired, so the recovery inbox only ever queries status IN
// ('candidate', 'medium_confidence', 'high_confidence') and never has to
// re-check dates.
...
export function useCandidateVisitTracking(userId) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeRecoveryChange(async (event) => {
      lastRefreshAt = 0
      if (event?.type === 'turned_off') await clearArrivalRecords()
      runRefresh()
    })
