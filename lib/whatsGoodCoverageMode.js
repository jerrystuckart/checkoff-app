 ...
 *   2026-09-23: this MUST be the authoritative full-inventory
 *   `eligibleLocalCount` lib/whatsGoodDataAdapter.js's
 *   assembleWhatsGoodCandidates() now returns (computed before Home Rail
 *   exclusion), never `candidates.length`/`ranked.length` — those measure a
 *   narrower post-personalization pool that can read zero even with real
 *   local inventory present. See that module's doc for the incident.
