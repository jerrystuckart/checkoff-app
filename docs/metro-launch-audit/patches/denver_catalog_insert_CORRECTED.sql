...
--
-- CORRECTED 2026-08-22 from denver_catalog_insert_draft.sql. One fix,
-- confirmed against the live linked DB before applying it here:
--   - `public.metros` -> `public.metro_areas` (the real table name; the
--     draft's `public.metros` does not exist in production at all —
--     confirmed via information_schema.tables — so the very first
--     statement in the draft would abort with
--     `relation "public.metros" does not exist` before writing anything).
--     Both occurrences fixed: the metro preflight guard below and the
--     post-insert verification SELECT further down.
-- Everything else in the first review pass — the 5 previously-unconfirmed
-- items/item_tags columns, the item_tags(item_id, tag_id) ON CONFLICT
-- target, and all 36 columns in the items INSERT — was independently
-- verified against the live schema and found correct as written.
--
-- SECOND EDIT PASS, 2026-08-22: applied the checkin_type='photo'
-- corrections identified during that same review. The candidate cohort
-- previously hardcoded checkin_type='tap' for all 149 rows via a literal
-- in the final INSERT SELECT; _denver_catalog_candidates now carries a
-- checkin_type column (one value per row) and the INSERT SELECT reads
-- c.checkin_type instead of the literal. 12 of 149 candidates are set to
-- 'photo', the other 137 remain 'tap':
--   Explicit "photo"/"photograph" language in body text (highest
--   confidence): DEN-SEE-008, DEN-SEE-026.
--   "Find [a specific object/detail]" locate-and-verify phrasing, no
--   literal "photo" word but same pattern (lower confidence, applied on
--   review): DEN-SEE-012, DEN-SEE-016, DEN-SEE-025, DEN-SEE-091,
--   DEN-SEE-103, DEN-SEE-105, DEN-SEE-106, DEN-SEE-114, DEN-SEE-117,
--   HG-002.
-- A weaker/more ambiguous "find" tier (DEN-SEE-002, DEN-SEE-006,
-- DEN-SEE-038, DEN-SEE-057, DEN-SEE-075 — "go look around" rather than
-- "locate one specific thing") was deliberately left as 'tap'.
-- photo_required is UNCHANGED (still false for every row, including the
-- 12 now-'photo' ones) — that's a related but separate field this pass
-- did not touch; flagged below, not applied.
--
-- Full reasoning and the complete body-text list (including the
-- weaker/ambiguous tier left as 'tap') is in
-- docs/metro-launch-audit/patches/denver_catalog_insert_review_notes.md
...
INSERT INTO _denver_catalog_candidates (
  ...
  ($co$DEN-SEE-095$co$, $co$McIntosh Lake$co$, $co$Complete the loop around 'McIntosh Lake'$co$, 'd1019850-5513-4d85-825c-7357ffc36ebc'::uuid, '018da150-6ebf-43f3-8eb1-e37338310738'::uuid, $co$tap$co$, $co$https://www.visitlongmont.org/things-to-do/$co$, $co$McIntosh Lake, Longmont, CO$co$, false, false, ARRAY[$co$adventure$co$, $co$outdoor$co$]::text[]),
...
-- Verification queries run before COMMIT. REWRITTEN 2026-08-22 (third
-- pass) to NOT depend on any _denver_* temp table surviving to this
-- point in the script. Root cause of the "_denver_required_neighborhoods
-- does not exist" error on the second run was never conclusively
-- isolated (couldn't reproduce it directly to confirm), but every one of
-- these queries can be made fully self-contained without losing
-- anything real: Denver had exactly 0 pre-existing items before this
-- script ever ran (confirmed by direct query beforehand), so ANY item
-- whose neighborhood_id falls under Denver's metro_id is unambiguously
-- part of this batch -- no join back to a candidates/reconciliation
-- temp table is needed to identify "this batch" at all. This removes
-- the fragility outright instead of relying on exactly why the earlier
-- version was fragile in this execution environment. The actual
-- correctness gates (the RAISE EXCEPTION checks above, which run
-- immediately adjacent to where their temp tables are created/used) are
-- UNCHANGED -- this rewrite only touches the read-only display queries
-- below them.
...
JOIN public.neighborhoods n ON n.id = i.neighborhood_id
WHERE n.metro_id = 'b00f7f91-3176-48c5-aaf1-6ded7426f756'::uuid
  AND i.google_place_id IS NOT NULL
...
JOIN public.items i ON i.id = cli.item_id
JOIN public.neighborhoods n ON n.id = i.neighborhood_id
WHERE n.metro_id = 'b00f7f91-3176-48c5-aaf1-6ded7426f756'::uuid;
