...
export function useNearby() {
  ...
  async function fetchItems() {
    // Paginated (see lib/supabasePagination.js) — a plain unbounded .select()
    // here silently truncates at PostgREST's default 1000-row cap once the
    // global eligible-item count crosses it, dropping real candidates from
    // the pool before distance is ever computed. .order('id') makes paging
    // deterministic (required for stable .range() pagination in Postgres).
    const { data: locationItems, error } = await fetchAllRows(() => supabase
      .from('items')
      .select(`
    ...
    const processed = rawItems
      .map(item => {
        ...
        return {
          ...
          // Nearby Redesign (2026-09-19) — this hook has no per-item
          // completion data of its own (that's a separate bulk fetch, see
          // lib/nearbyCompletedItems.js) — this is a placeholder value,
          // always overwritten downstream by DiscoverScreen's displayItems
          // pipeline with the real completedItemIds Set before render.
          checked:          false,
          isUniversal:      false,
